...
```{r}
demographics = read_csv(file = 'DAT_exp2_demographics_cleaned.csv') %>%
...
```{r}
demographics %>%
  summarise(m_age = mean(as.numeric(Q12), na.rm = TRUE),
            sd_age = sd(as.numeric(Q12), na.rm = TRUE))
```
...
d = d %>%
  ...
  summarize(response_list = list(response_word)) %>%
  left_join(d) %>%
...
d = d %>%
  ...
  # Code position for analyses
  mutate(list_position_num = recode(recall_position, 
                                    '6' = 0.5, 
                                    '5' = 0.3, 
                                    '4' = 0.1, 
                                    '3' = -0.1, 
...
In the following analyses, we care about reversing the coding. Whereas before we were looking at whether the recalled word matched a given position in the studied list, here we care about whether the studied word matches a word in the recalled list. While this coding scheme should result in the same thing when looking at all positions, if we only look at a subset of positions (i.e. 4, 5, 6), as we do in a later model, then this will be important.
...
```{r,  results = 'hide', message = FALSE}
d = d %>%
  mutate(list_condition_num = ifelse(list_condition == 'intransitive', -0.5, 0.5),
...
write_tsv(d, path = "DAT_exp2_dataframe.tsv")
```

Load dataframe

```{r}
d = read_tsv(file = "DAT_exp2_dataframe.tsv")
```
...
```{r}
# 716 x 600
d_strict_position_summary_within = d %>%
  summarySEwithin(measurevar = "correct_strict",
...
d_strict_position_summary_within %>%
  mutate(list_condition = str_to_title(list_condition)) %>%
...
Item memory -- Lenient scoring

```{r}
d_lenient_summary_within = d %>%
  summarySEwithin(measurevar = "correct_lenient_studied",
                  withinvars = "list_condition",
                  idvar = "participant_id",
                  na.rm = FALSE,
                  conf.interval = .95)

d_lenient_summary_within %>%
  mutate(list_condition = str_to_title(list_condition)) %>%
  ggplot(aes(x = list_condition, 
             y = correct_lenient_studied,
             fill = list_condition)) +
  geom_bar(stat = "identity") +
  geom_errorbar(aes(ymin = correct_lenient_studied - ci, ymax = correct_lenient_studied + ci), 
                width = 0.1) +
  scale_y_continuous(limits = c(0, 1.0),
                     expand = c(0, 0)) +
  scale_x_discrete(limits = rev) +
...
And also considering position

```{r}
# 716 x 600
d_lenient_position_summary_within = d %>%
  summarySEwithin(measurevar = "correct_lenient_studied",
                  withinvars = c("list_condition", "recall_position"),
                  idvar = "participant_id",
                  na.rm = FALSE,
...
d_lenient_position_summary_within %>%
  mutate(list_condition = str_to_title(list_condition)) %>%
  ggplot(aes(x = recall_position, y = correct_lenient_studied, color = list_condition, group = list_condition)) +
  geom_point(size = 1.5) +
  geom_line(size = 1.5) +
  geom_errorbar(aes(ymin = correct_lenient_studied - ci, ymax = correct_lenient_studied + ci), 
                size = 1.0,
...
Order memory -- Only for items that have been recalled correctly in any position
...
d %>%
  ...
  ggplot(aes(x = list_condition, 
             y = correct_lenient,
             fill = list_condition)) +
  stat_summary(geom = 'bar', alpha = 0.75, size = 2) +
  stat_summary(geom = 'errorbar', alpha = 0.5, width = .15) +
  scale_y_continuous(limits = c(0, 1.0)) +
  labs(x = "List position", 
       y = "Proportion correct", 
       color = "Recall condition",
...
### Accuracy in the last 3 positions
...
```{r}
m_strict_final = glmer(correct_strict ~ 
                         list_condition_num + 
                         frame_condition_num + 
                       ...
                       data = d_final_positions,
                       family = binomial)
summary(m_strict_final)
Anova(m_strict_final, type = 3)
```

While also considering position

```{r}
m_strict_final_position = glmer(correct_strict ~
                                  list_condition_num +
                                  frame_condition_num +
                                  list_condition_num:frame_condition_num +
...
summary(m_strict_final_position)
Anova(m_strict_final_position, type = 3)
```

Accuracy overall: NOT LOOKING AT JUST LAST 3 POSITIONS

```{r}
m_strict_all = glmer(correct_strict ~ 
                         list_condition_num + 
                         frame_condition_num + 
                         list_condition_num:frame_condition_num +
                         (1 + list_condition_num + frame_condition_num|crit_pair_f) + 
                       ...
                       data = d,
                       family = binomial)
summary(m_strict_all)
Anova(m_strict_all, type = 3)
```

Accuracy overall while considering position: NOT LOOKING AT JUST LAST 3 POSITIONS

```{r}
m_strict_all_position = glmer(correct_strict ~
                                  list_condition_num +
                                  frame_condition_num +
                                  list_condition_num:frame_condition_num +
                                  list_position_num +
                                ...
                                control = glmerControl(optimizer = 'bobyqa',
                                                     optCtrl = list(maxfun = 50000)))
summary(m_strict_all_position)
Anova(m_strict_all_position, type = 3)
```

### Item analyses

We are looking at whether the words studied in position 4, 5, and 6 are recalled in any position throughout the list. Note, the name of the variable is misleading. When using the `correct_studied_lenient` variable, the `recall_position` variable reflects the studied word.

```{r}
d_final_positions_item = d %>%
  filter(recall_position %in% c(4, 5, 6))
```

```{r}
m_binary_final_item = glmer(correct_lenient_studied ~
                              list_condition_num +
                              frame_condition_num +
                              list_condition_num:frame_condition_num +
                              ...
                              (1|participant_id),
                            data = d_final_positions_item,
                            family = binomial,
                            control = glmerControl(optCtrl = list(maxfun = 50000)))
summary(m_binary_final_item)
Anova(m_binary_final_item, type = 3)
```

While also considering position

```{r}
m_binary_final_item_position = glmer(correct_lenient_studied ~
                                       list_condition_num +
                                       frame_condition_num +
                                       list_condition_num:frame_condition_num +
                                       recall_position_ranged +
                                     ...
                                     control = glmerControl(optimizer = 'bobyqa',
                                                            optCtrl = list(maxfun = 50000)))
summary(m_binary_final_item_position)
Anova(m_binary_final_item_position, type = 3)
```

Item memory: NOT JUST LOOKING AT LAST 3 POSITIONS

```{r}
m_binary_all_item_position = glmer(correct_lenient_studied ~
                                       list_condition_num +
                                       frame_condition_num +
                                       list_condition_num:frame_condition_num +
                                       list_position_num +
                                     ...
                                     control = glmerControl(optimizer = 'bobyqa',
                                                            optCtrl = list(maxfun = 50000)))
summary(m_binary_all_item_position)
Anova(m_binary_all_item_position, type = 3)
```

### Order propensity analyses

For order analyses, we only look at the items in studied position 4, 5, and 6 are recalled in the correct position, only for the cases where those items had been recalled at all. So, we are just looking at whether or not the studied word is recalled in the correct position.

```{r}
d_final_positions_order = d %>%
  filter(recall_position %in% c(4, 5, 6)) %>%
  filter(correct_lenient_studied == 1)

d_all_positions_order = d %>%
  filter(correct_lenient_studied == 1)
```

```{r}
m_binary_final_order = glmer(correct_strict ~ 
                         list_condition_num + 
                         frame_condition_num + 
                         list_condition_num:frame_condition_num +
                         (1 + list_condition_num + frame_condition_num|crit_pair_f) + 
                         (1 + list_condition_num|participant_id),
                       data = d_final_positions_order,
                       family = binomial)
summary(m_binary_final_order)
Anova(m_binary_final_order, type = 3)
```

While also considering position

```{r}
m_binary_final_order_position = glmer(correct_strict ~
                                       list_condition_num +
                                       frame_condition_num +
                                       list_condition_num:frame_condition_num +
                                       recall_position_ranged +
                                     ...
                                     control = glmerControl(optimizer = 'bobyqa',
                                                            optCtrl = list(maxfun = 50000)))
summary(m_binary_final_order_position)
Anova(m_binary_final_order_position, type = 3)
```

Order propensity: NOT JUST THE LAST 3 POSITIONS

```{r}
m_binary_all_order_position = glmer(correct_strict ~
                                       list_condition_num +
                                       frame_condition_num +
                                       list_condition_num:frame_condition_num +
                                       list_position_num +
                                       ...
                                       (1 + list_condition_num + frame_condition_num + list_condition_num:frame_condition_num|crit_pair_f) +
                                       (1 + list_condition_num|participant_id),
                                     data = d_all_positions_order,
                                     family = binomial,
                                     control = glmerControl(optimizer = 'bobyqa',
                                                            optCtrl = list(maxfun = 50000)))
summary(m_binary_all_order_position)
Anova(m_binary_all_order_position, type = 3)
```