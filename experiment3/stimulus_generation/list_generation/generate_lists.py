...
def read_critical_subjects(path_stimuli,
                           animate_key = 'animate_noun',
                           inanimate_key = 'inanimate_noun'):
    """
    """
    stim = pandas.read_csv(path_stimuli)
    animate_nouns = list(stim[animate_key])
    inanimate_nouns = list(stim[inanimate_key])

    return [{'animate': animate, 'inanimate': inanimate} for animate, inanimate in zip(animate_nouns, inanimate_nouns)]
...
def generate_list_conditions(num_lists,
                             possible_conditions = ['animate', 'inanimate']):
    """
    """
...
def generate_trial_frame(stimuli,
                         trial_order = ['ADJ', 'CRIT', 'VERB', 'ADJ', 'AN', 'IN']):
    """
    """
    trial = []
...
if __name__ == '__main__':
    ...
    path_subjects = Path.cwd().joinpath('stimuli/paired_nouns.csv')
    ...
    path_verbs = Path.cwd().joinpath('stimuli/transitive_verbs.csv')

    subjects = read_critical_subjects(path_stimuli = path_subjects)
    ...
    verbs = read_context(path_stimuli = path_verbs)
