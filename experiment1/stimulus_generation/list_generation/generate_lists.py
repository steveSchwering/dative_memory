...
def read_critical_vns(path_stimuli,
                      verb_key = 'critical_verb',
                      noun_key = 'critical_noun'):
    """
    """
    stim = pandas.read_csv(path_stimuli)
    verbs = list(stim[verb_key])
    nouns = list(stim[noun_key])

    return [{'verb': verb, 'noun': noun} for verb, noun in zip(verbs, nouns)]
...
if __name__ == '__main__':
    ...
    path_vns = Path.cwd().joinpath('stimuli/critical_vn.csv')
    ...
    crits = read_critical_vns(path_stimuli = path_vns)
